...
with open(file_dir + "/../../config.json") as json_data_file:
    config = json.load(json_data_file)

class Pontuacao:
    def generate(nome):
        ponderado = pd.read_csv(file_dir + '/../' + nome + '_ponderado.csv', dtype = numpy.float64, sep =';')
        ...
        df.to_csv(file_dir + '/../' + nome + '_pontuacao.csv', sep =';', index = False)
